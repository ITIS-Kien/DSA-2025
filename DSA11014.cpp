...
void calculate(tree root){
    if(root == NULL) return;
    if(root->right != NULL and isLeaf(root->right)) SumRight += root->right->value;
    else if(root->right != NULL) calculate(root->right);
    if(root->left != NULL and !isLeaf(root->left)) calculate(root->left);
}
signed main(){
    nguyentukien_218
    int t; cin >> t;
